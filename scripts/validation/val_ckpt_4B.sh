...
DP_SIZE=1              # Data Parallel Size（4B 单卡可跑，用 DP 提高吞吐）
...
MAX_CONCURRENT="${3:-${MAX_CONCURRENT:-10}}"
...
# 在后 4 张 GPU（4,5,6,7）上启动 SGLang，与前 4 卡（如 30B 训练）隔离
...
# 日志文件名带 UUID 后缀，避免多次运行互相覆盖
SGLANG_LOG="sglang_server_$(uuidgen 2>/dev/null | cut -c1-8 || cat /proc/sys/kernel/random/uuid 2>/dev/null | cut -c1-8 || echo "$$$(date +%s)").log"
# 在后台启动sglang服务器
eval "${SGLANG_CMD}" > "${SGLANG_LOG}" 2>&1 &

SGLANG_PID=$!
echo "SGLang服务器已启动，PID: ${SGLANG_PID}"
echo "日志文件: ${SGLANG_LOG}"
...
VALIDATION_CMD="${VALIDATION_CMD} --thinking_model False"
VALIDATION_CMD="${VALIDATION_CMD} --enable_thinking False"
VALIDATION_CMD="${VALIDATION_CMD} --enable_logp True"
