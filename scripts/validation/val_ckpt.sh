...
ENABLE_LOGP="False"
AVG_N="8"                # 留空则由 Python 自动推断（thinking 模式默认 8，否则 1）
...
# 验证结束后是否释放 SGLang：1/yes=释放并结束进程；0/no=不释放，保留服务（起完之后不要停）
CLEANUP_SGLANG="${CLEANUP_SGLANG:-0}"
...
# 设置清理函数（CLEANUP_SGLANG 非 1/yes 时不杀进程，仅提示）
cleanup () {
    echo ""
    if [ "${CLEANUP_SGLANG}" != "1" ] && [ "${CLEANUP_SGLANG}" != "yes" ] && [ "${CLEANUP_SGLANG}" != "true" ]; then
        echo "========== 保留 SGLang（起完之后不要停）=========="
        echo "SGLang 继续运行于端口 ${PORT}，可手动结束或用于后续请求。"
        return 0
    fi
    echo "========== 清理 SGLang 进程 =========="
